...
export default function AcademicYearsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingYear) {
        await apiFetch(`/academic-years/${editingYear.id}`, {
          method: "PATCH",
      ...
      setError(err instanceof ApiError ? err.message : "Failed to save academic year");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const canCreate = user && user.permissions.includes("academic_year:create");
  const canUpdate = user && user.permissions.includes("academic_year:update");
  const canDelete = user && user.permissions.includes("academic_year:delete");
  const canManage = user && user.permissions.includes("academic_year:manage");
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage academic years and their status</p>
        </div>
        {(canCreate || canManage) && (
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-500">No academic years found</td>
                </tr>
              ) : (
                years.map((year) => (
                  <tr key={year.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-sm text-gray-500">Created {new Date(year.createdAt).toLocaleDateString()}</div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      {year.isCurrent ? (
                        <span className="px-2 py-1 text-xs font-medium bg-blue-100 text-blue-800 rounded-full">Yes</span>
                      ) : (
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold text-gray-900">{editingYear ? "Edit Academic Year" : "Create Academic Year"}</h2>
