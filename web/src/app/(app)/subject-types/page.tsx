...
export default function SubjectTypesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingType) {
        await apiFetch(`/subject-types/${editingType.id}`, {
          method: "PATCH",
          body: JSON.stringify(formData),
        });
      } else {
        await apiFetch("/subject-types", {
          method: "POST",
          body: JSON.stringify(formData),
        });
      }
      ...
      setError(err instanceof ApiError ? err.message : "Failed to save subject type");
    } finally {
      setSubmitting(false);
    }
  };
  const handleDelete = async (id: string) => {
    ...
    try {
      await apiFetch(`/subject-types/${id}`, { method: "DELETE" });
      fetchSubjectTypes();
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "Failed to delete subject type");
    }
  };
  ...
  const openEditModal = (type: SubjectType) => {
    setEditingType(type);
    setFormData({
      name: type.name,
  ...
  const canCreate = user && user.permissions.includes("subject_type:create");
  const canUpdate = user && user.permissions.includes("subject_type:update");
  const canDelete = user && user.permissions.includes("subject_type:delete");
  const canManage = user && user.permissions.includes("subject_type:manage");
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Subject Types</h1>
          <p className="text-gray-500 mt-1">Manage subject categories (Core, Elective, Language, etc.)</p>
        </div>
        {(canCreate || canManage) && (
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan={3} className="px-6 py-12 text-center text-gray-500">No subject types found</td>
                </tr>
              ) : (
                subjectTypes.map((type) => (
                  <tr key={type.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-sm text-gray-500">Created {new Date(type.createdAt).toLocaleDateString()}</div>
                    </td>
                    <td className="px-6 py-4 text-gray-900">{type.description || "—"}</td>
                    {(canUpdate || canManage || canDelete) && (
                      <td className="px-6 py-4">
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold text-gray-900">{editingType ? "Edit Subject Type" : "Create Subject Type"}</h2>
